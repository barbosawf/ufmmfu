...
#' @param fun Modified non-linear function with `.args` prefix (required for internal computation) and suffixes that refer to the parameters of the non-linear model used. Example: if the logistic model was used as `function(x, Asym, xmid, scal) {Asym / (1 + exp(-scal * (x - xmid)))}` in the `nlme::nlsList()` function, the argument `fun` will be `function(x, .args) {.args$Asym / (1 + exp(-.args$scal * (x - .args$xmid)))}`.
...
#'
#' @return Return a list and a panel of ggplo2-made graphics from the object generated by the `nlme::nlsList()` function. Non-linear curves are smoothed by using the `ggplot2::stat_function()` function.
#'
#' @examples
#'
#' df <- datasets::Orange
